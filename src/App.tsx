...
function App() {
  ...
  const { volumes, setTrackVolume, applyPreset, toggleMasterPlay, isPlaying, masterVolume, setMasterVolume } = useAudioMixer(phase);
        ...
        <header className="p-6 shrink-0 backdrop-blur-sm sticky top-0 z-50 space-y-4 bg-gradient-to-b from-black/80 to-transparent">
          <div className="flex justify-between items-center">
            <div className="flex flex-col">
              ...
              <p className="text-[10px] uppercase tracking-widest text-white/50">Binaural Engine</p>
            </div>
            <button 
          ...
          <div className="flex items-center gap-3 px-1 pt-1">
            <button onClick={() => setMasterVolume(masterVolume > 0 ? 0 : 0.8)} className="text-white/70 hover:text-white">
            ...
            <div className="relative flex-1 h-6 flex items-center group">
                <input type="range" min="0" max="1" step="0.01" value={masterVolume} onChange={(e) => setMasterVolume(parseFloat(e.target.value))} className="w-full z-10 opacity-0 cursor-pointer absolute inset-0 h-full" />
                <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden relative">
                    <div className="h-full bg-white/80 transition-all duration-100 ease-out" style={{ width: `${masterVolume * 100}%` }} />
                </div>
                <div className="h-4 w-4 bg-white rounded-full shadow-lg absolute pointer-events-none transition-all duration-100 ease-out group-active:scale-110" style={{ left: `calc(${masterVolume * 100}% - 8px)` }} />
            </div>
          </div>
        </header>

        <main className="flex-1 flex flex-col max-w-md mx-auto w-full px-6 pb-32">
          <div className="py-8 shrink-0"><FocusTimer onPhaseChange={setPhase} /></div>

          <div className="mb-8 shrink-0">
            <div className="flex items-center gap-2 mb-4 text-white/60 px-1">
              <LayoutTemplate className="w-4 h-4" />
              <span className="text-xs font-bold uppercase tracking-widest">Brain States</span>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between px-1 mb-2">
              <div className="flex items-center gap-2 text-white/60">
                ...
                <span className="text-xs font-bold uppercase tracking-widest">Frequencies</span>
              </div>
              {phase === 'break' && <span className="text-[10px] bg-emerald-500/20 text-emerald-200 px-2 py-0.5 rounded animate-pulse">Rest Mode</span>}
            </div>
            {SOUNDS.map(sound => (
              <SoundControl key={sound.id} sound={sound} volume={volumes[sound.id]} onChange={(val) => { setTrackVolume(sound.id, val); if (val > 0 && !isPlaying) toggleMasterPlay(); }} />
            ))}
          </div>
        </main>
      </div>
